    ...
    abstract internal class Part : CCSprite
    {
        ...
        internal IEnumerable<Part> TotalParts
        {
            get
            {
                ...
                foreach (PartMount partAnchor in PartMounts)
                {
                    if (partAnchor.MountedPart != null)
                        totalParts.AddRange(partAnchor.MountedPart.TotalParts);
                }
                return totalParts;
            }
        }
        ...
        internal bool MountPart (int mountIndex, Part part, int dz = 1)
        {
            ...
            // if it was sucessfully mounted add it as a child
            if (mounted)
                AddChild(part, zOrder: this.ZOrder + dz);
            return mounted;
        }
